...
export async function createWorkspace(formData: FormData) {
  ...
  if (organizationName.length < 2 || siteName.length < 1) {
    onboardingError("Informe o nome da empresa e o nome do primeiro local.");
  }
  ...
  redirect("/dashboard?message=Ambiente%20criado%20com%20sucesso.");
}
...
export async function createFirstSite(formData: FormData) {
  ...
  const siteName = String(formData.get("site_name") ?? "").trim();
  const timezone = safeTimezone(String(formData.get("timezone") ?? "America/Sao_Paulo"));
  if (!siteName) onboardingError("Informe o nome do local.");
  ...
  revalidatePath("/dashboard");
  redirect("/dashboard?message=Local%20criado%20com%20sucesso.");
}
