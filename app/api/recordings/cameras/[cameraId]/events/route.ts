...
const TimelineSchema = z.object({
  offsetMs: z.number().int().min(0).max(3_600_000).optional(),
  sourceTimestamp: IsoDateSchema.optional(),
  source: z.literal("local_recording").optional(),
}).passthrough();
...
const LocalMetricsSchema = z.object({
  ...
  framesObserved: z.number().int().min(1).max(1_000_000),
    ...
    return NextResponse.json(
      {
        ...
        type: event
          ? String(event.primary_event_type)
          : "no_relevant_change",
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const authenticated = await authorizeRecordingCamera(cameraId, {
    requireManager: true,
  });
  ...
  const { data: recordingSession, error: sessionError } =
    await authenticated.supabase
      .from("recording_sessions")
      .select(
        "id,status,source_started_at,duration_seconds,source_filename",
      )
      .eq("id", input.recordingSessionId)
      .eq("organization_id", authenticated.organization.id)
  ...
  if (
    sessionError ||
    !recordingSession ||
    !["reserved", "processing"].includes(String(recordingSession.status))
  ) {
    return NextResponse.json(
  ...
  const sessionStart = Date.parse(String(recordingSession.source_started_at));
  const sessionEnd =
    sessionStart + Number(recordingSession.duration_seconds) * 1000 + 2000;

  if (
    !Number.isFinite(sessionStart) ||
    startedAt.getTime() < sessionStart - 2000 ||
    endedAt.getTime() > sessionEnd
  ) {
    return NextResponse.json(
      { ok: false, error: "event_outside_recording_session" },
      { status: 400 },
    );
  }

  const labels = new Set(input.frames.map((frame) => frame.label));
  ...
  // Limite de proteção: no máximo quatro acontecimentos por minuto de vídeo.
  // A franquia limita horas; este teto independente impede um arquivo ruidoso
  // de virar uma sequência praticamente ilimitada de chamadas de IA.
  const maximumEvents = Math.max(
    1,
    Math.ceil(Number(recordingSession.duration_seconds) / 15),
  );
  const { count: currentEventCount, error: eventCountError } = await supabase
    .from("analysis_jobs")
    .select("id", { count: "exact", head: true })
    .eq("recording_session_id", input.recordingSessionId);

  if (eventCountError) {
    return NextResponse.json(
      { ok: false, error: "recording_event_limit_unavailable" },
      { status: 500 },
    );
  }

  if ((currentEventCount ?? 0) >= maximumEvents) {
    return NextResponse.json(
      {
        ok: false,
        error: "recording_event_limit_reached",
        maximumEvents,
      },
      { status: 429 },
    );
  ...
  try {
    preliminaryContext = await buildFrozenEventAnalysisContext({
      authenticated,
      cameraId,
    });
  } catch (contextError) {
    ...
    return NextResponse.json(
      { ok: false, error: code },
      { status: code === "active_camera_profile_required" ? 409 : 500 },
    );
  }

  const requestedPlanCode = normalizeAnalysisPlan(
  ...
  const { data: job, error: jobError } = await supabase
    ...
    .insert({
      ...
      local_metrics: recordingMetrics,
    ...
    .select(
      "id,analysis_plan_code,trial_run_id,retention_snapshot",
    )
    .single();
  ...
  const retentionSnapshot =
    job.retention_snapshot &&
    typeof job.retention_snapshot === "object" &&
    !Array.isArray(job.retention_snapshot)
      ? (job.retention_snapshot as Record<string, unknown>)
      : null;

  const frozenContext = applyEffectivePlanToFrozenContext(
    preliminaryContext,
    {
  ...
  const priority =
    effectiveVisionPlan.maximumFrames === 1
      ? ["peak", "start", "end", "extra"]
      : ["start", "peak", "end", "extra"];
  const priorityIndex = new Map(
    priority.map((label, index) => [label, index]),
  );
  const effectiveDecoded = [...decoded]
    .sort(
      (left, right) =>
        (priorityIndex.get(left.label) ?? 99) -
        (priorityIndex.get(right.label) ?? 99),
    )
  ...
  const { data: alignedJob, error: alignError } = await supabase
    ...
    .update({
      ...
      local_metrics: effectiveMetrics,
    })
    .eq("id", analysisJobId)
    .eq("status", "queued")
    .select("id")
  ...
  // Gravações históricas mantêm o horário original no evento, mas a retenção
  // começa na importação para não nascer expirada.
  const retentionAnchor = Date.now();
  const temporaryExpiresAt = new Date(
    retentionAnchor +
      frozenContext.retention.temporaryFrameDays * 86_400_000,
  ).toISOString();
  ...
  if (ingestionError || !ingestion) {
    await supabase
      ...
      .eq("status", "queued");
    return NextResponse.json(
      { ok: false, error: "event_ingestion_create_failed" },
      { status: 500 },
    );
  }
  ...
  try {
    for (const frame of effectiveDecoded) {
      ...
      if (assetError || !asset) {
        throw assetError ?? new Error("asset_missing");
      }
    ...
    if (readyError || !readyRow) {
      throw readyError ?? new Error("event_ingestion_ready_transition_failed");
    }
  } catch (error) {
    ...
    console.error(
      `Falha ao persistir evidência de gravação ${ingestionId}:`,
      message.slice(0, 800),
    );
    ...
    return NextResponse.json(
      { ok: false, error: "event_evidence_persist_failed" },
      { status: 500 },
    );
  }
  ...
  after(async () => {
    try {
      ...
      console.error(
        `Processamento de gravação pós-ACK falhou para ${ingestionId}; recovery assumirá:`,
        error instanceof Error ? error.message : String(error),
      );
    }
  });
