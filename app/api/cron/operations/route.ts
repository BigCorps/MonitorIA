...
export async function GET(request: NextRequest) {
  ...
  const alertResult = await supabase.rpc("refresh_operational_alerts_v1", {
    p_recommended_agent_version: process.env.AGENT_RECOMMENDED_VERSION?.trim() || "0.9.0",
  });
  ...
  const { error: finishError } = await supabase.from("operational_refresh_runs").update({
    status: "completed",
    alerts_opened: Number(alerts.opened ?? 0),
    alerts_resolved: Number(alerts.resolved ?? 0),
    journeys_created: journeyCount,
    finished_at: finishedAt,
    metadata: { alertsActive: Number(alerts.active ?? 0) },
  }).eq("id", run.id);
  ...
  return NextResponse.json(
    { ok: true, alerts, journeys, finishedAt },
