...
const HeartbeatSchema = z.object({
  ...
  queuedEvents: z.number().int().nonnegative().max(1_000_000).default(0),
...
export async function POST(request: NextRequest) {
  ...
  const metadata = { ...agent.metadata, ...(body.metadata ?? {}) };
  ...
  if (updateError) {
    console.error("Falha ao atualizar heartbeat do Agent:", updateError.message);
    return NextResponse.json({ ok: false, error: "heartbeat_failed" }, { status: 500 });
  }

  const { error: healthError } = await supabase.from("agent_health").insert({
  ...
  if (healthError) {
    console.error("Falha ao registrar saúde do Agent:", healthError.message);
  }

  return NextResponse.json(
    { ok: true, agentId: agent.id, serverTime: now },
