...
export default function TermosPage() {
  return (
    <MarketingPage
      ...
      lead="Última atualização: 7 de agosto de 2026. Ao contratar ou utilizar o serviço, a organização declara ter lido e aceito estes termos e as condições comerciais aplicáveis."
    >
      <ContentSection label="Termos" title="Regras gerais do serviço.">
        <Prose>
          <h2>1. Identificação do fornecedor</h2>
          <p>O MonitorIA.cam é fornecido por {appConfig.legal.legalName}, nome fantasia {appConfig.legal.tradeName}, CNPJ {appConfig.legal.taxId}, com endereço em {appConfig.legal.address}. O representante legal é {appConfig.legal.legalRepresentative.name}, {appConfig.legal.legalRepresentative.role}. O contato institucional é <a href={`mailto:${appConfig.legal.legalEmail}`}>{appConfig.legal.legalEmail}</a> ou <a href={appConfig.legal.institutionalPhoneHref}>{appConfig.legal.institutionalPhone}</a>.</p>
          ...
          <h2>3. Não substituição da segurança</h2>
          ...
          <h2>4. Limites da inteligência artificial</h2>
          ...
          <h2>8. Planos, limites e pagamento</h2>
          ...
          <p>Estes termos são regidos pela legislação brasileira. Fica eleito o {appConfig.legal.jurisdiction}, observadas as regras legais de competência aplicáveis.</p>

          <Note>Propostas, pedidos, planos e contratos específicos podem complementar estes termos quanto a preço, SLA, implantação, suporte, limites e cancelamento.</Note>
        </Prose>
      </ContentSection>
    </MarketingPage>
  );
