-- MonitorIA — rollback da Fase 3
-- Remove sessões operacionais e restaura a pesquisa de eventos da Fase 2.
...
drop function if exists private.process_monitoria_operational_session_event();
drop function if exists public.process_operational_session_v1(uuid);
drop function if exists public.finalize_stale_operational_sessions_v1(uuid, uuid);
drop function if exists public.search_operational_sessions(
  uuid, timestamptz, timestamptz, uuid, uuid, text, text, integer, integer
);
drop function if exists public.assistant_operational_sessions_summary(
  uuid, timestamptz, timestamptz, uuid, uuid
);
drop function if exists private.monitoria_session_type_rank(text);
...
begin
  if exists (
    ...
    alter publication supabase_realtime
      drop table public.operational_sessions;
  end if;
end
$$;

alter table public.events
  drop constraint if exists events_operational_session_id_fkey,
  drop constraint if exists events_session_confidence_check,
  drop constraint if exists events_session_summary_check;

alter table public.interaction_groups
  drop constraint if exists interaction_groups_operational_session_id_fkey;

alter table public.events
  drop column if exists operational_session_id,
  drop column if exists session_type,
  drop column if exists session_status,
  drop column if exists session_chapter_type,
  drop column if exists session_chapter_order,
  drop column if exists session_chapter_count,
  drop column if exists session_duration_seconds,
  drop column if exists session_confidence,
  drop column if exists session_summary;

alter table public.interaction_groups
  drop column if exists operational_session_id;

drop table if exists public.operational_session_outcomes cascade;
drop table if exists public.operational_session_participants cascade;
drop table if exists public.operational_session_events cascade;
drop table if exists public.operational_session_groups cascade;
drop table if exists public.operational_sessions cascade;

alter table public.cameras
  drop constraint if exists cameras_session_idle_close_check,
  drop constraint if exists cameras_session_max_duration_check,
  drop constraint if exists cameras_session_min_confidence_check;

alter table public.cameras
  drop column if exists operational_sessions_enabled,
  drop column if exists session_idle_close_minutes,
  drop column if exists session_max_duration_minutes,
  drop column if exists session_min_confidence;
