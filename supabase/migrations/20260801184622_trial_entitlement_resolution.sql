-- A assinatura de retorno muda nesta fase; a view dependente precisa ser
-- recriada junto com a função.
drop view if exists public.camera_entitlements;
drop function if exists public.resolve_camera_entitlement(uuid);
...
begin
  ...
  if not v_enforcement then
    ...
    v_assistant_allowed :=
      v_trial.status in ('running', 'capture_completed', 'exploration')
    ...
    elsif v_trial.status = 'expired' then
      v_reason := 'trial_expired';
    elsif v_trial.status = 'purged' then
      v_reason := 'trial_data_purged';
    elsif v_trial.status in ('draft', 'ready') then
...
select
  ...
  entitlement.capture_ends_at,
  entitlement.exploration_ends_at,
  entitlement.purge_after,
  entitlement.metadata_retention_days,
  entitlement.long_term_keyframes,
  entitlement.temporary_frame_days,
  entitlement.maximum_analysis_frames,
  entitlement.maximum_escalation_percent,
  ...
  entitlement.enforcement_enabled,
  entitlement.reason
from public.cameras camera
