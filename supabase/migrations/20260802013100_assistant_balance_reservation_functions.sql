-- MonitorIA Fase 6 — saldo, reserva, conclusão e expiração

create or replace function private.assistant_commercial_enforcement_enabled(
...
as $$
  select coalesce(
    (
      select account.entitlement_enforcement_enabled
...
revoke all on function private.assistant_commercial_enforcement_enabled(uuid)
from public, anon, authenticated;
grant execute on function private.assistant_commercial_enforcement_enabled(uuid)
to service_role;
...
declare
  ...
  v_source text := 'none';
begin
  if not v_is_service and not private.is_org_member(p_organization_id) then
...
begin
  ...
  if v_usage.status = 'completed' then
    return jsonb_build_object(
      ...
      'duplicate', true,
      'balance', public.get_assistant_balance(v_usage.organization_id)
    );
  end if;
...
create or replace function private.release_assistant_interaction(
  p_usage_event_id uuid,
...
begin
  ...
  set status = 'released',
      released_at = now(),
      release_reason = left(coalesce(p_reason, 'request_failed'), 160),
      reservation_expires_at = null,
      metadata = metadata || coalesce(p_metadata, '{}'::jsonb)
...
revoke all on function private.release_assistant_interaction(uuid, text, jsonb)
from public, anon, authenticated;
grant execute on function private.release_assistant_interaction(uuid, text, jsonb)
to service_role;

create or replace function public.expire_assistant_reservations()
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  return jsonb_build_object(
    'success', true,
    'releasedReservations', v_released
  );
end;
$$;

revoke all on function public.expire_assistant_reservations()
from public, anon, authenticated;
grant execute on function public.expire_assistant_reservations()
to service_role;

create or replace function public.process_assistant_credit_expirations()
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  for v_purchase in
    ...
    for update
  loop
    if v_purchase.remaining_interactions > 0 then
      insert into public.assistant_credit_ledger (
        organization_id,
        purchase_id,
        delta_interactions,
      ...
      values (
        ...
        v_purchase.id,
        -v_purchase.remaining_interactions,
        'assistant_credit_pack_expired',
        0
      );
    end if;

    update public.assistant_credit_purchases
    set remaining_interactions = 0,
        status = 'expired',
        expired_at = now()
    where id = v_purchase.id;
  ...
  where status = 'active'
    and remaining_interactions = 0;

  get diagnostics v_consumed = row_count;

  return jsonb_build_object(
    'success', true,
    'expiredPurchases', v_expired,
    'consumedPurchases', v_consumed
  );
end;
$$;

revoke all on function public.process_assistant_credit_expirations()
from public, anon, authenticated;
grant execute on function public.process_assistant_credit_expirations()
to service_role;
