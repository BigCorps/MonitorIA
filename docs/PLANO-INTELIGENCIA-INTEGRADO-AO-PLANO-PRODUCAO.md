...
Documento de coordenação entre agentesData de consolidação: 1º de agosto de 2026Produto-base: MonitorIA.cam v0.8.2Meta comercial: MonitorIA.cam v1.0.0Responsável pelo produto: BigCorps TecnologiaEscopo deste documento: inteligência visual, memória operacional, Assistente e MCP
...
o que já foi preparado na trilha de inteligência;

o que ainda será desenvolvido;

como cada entrega se encaixa nas fases oficiais de produção;

o que pertence a outros agentes;

quais arquivos, tabelas e contratos exigem coordenação;

como evitar implementações duplicadas ou incompatíveis.
...
Realtime da linha do tempo

Preparado

Realtime de events habilitado via MCP

Não confirmado

INT-3

Operational Sessions v1
...
Multi-scene, routing e vehicle memory
...
Serviço, RTSP, fila e update

Assistente

Intenções e evidências
...
OAuth operacional e submissão

Clipes

Relevância e janela

Buffer, encode e Storage
...
eventos reais forem estruturados;

estados forem acompanhados;

eventos próximos forem agrupados;

sessões forem reconstruídas;

cenas simples e complexas usarem rotas adequadas;

custos forem mensuráveis;

pessoas e veículos forem apenas prováveis;

não houver reconhecimento facial;

Assistente responder com evidência;

MCP usar contratos estáveis;

Agent fornecer métricas suficientes;

incertezas forem explícitas;

RLS impedir acesso cruzado;

retention respeitar planos;

alertas reutilizarem a mesma base;

homologação for aprovada.

18. Próxima ação oficial

Antes de iniciar INT-4, confirmar:

[ ] INT-1 aplicada no Supabase
[ ] INT-1 aplicada no repositório
[ ] INT-2 aplicada no Supabase
[ ] INT-2 aplicada no repositório
[ ] INT-2.5 frontend aplicado
[ ] INT-3 aplicada no Supabase
[ ] INT-3 aplicada no repositório
[ ] INT-3.5 aplicada no Supabase
[ ] INT-3.5 aplicada no repositório
[ ] INT-3.8 aplicada no Supabase
[ ] INT-3.8 aplicada no repositório
[ ] build aprovado
[ ] deploy aprovado
