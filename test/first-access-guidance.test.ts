...
import { CURRENT_AGENT_VERSION, DEFAULT_INSTALLER_URLS, installerUrlFor } from "../src/lib/installer-data.js";
...
test("primeiro acesso mostra opções de instalação junto do código", async () => {
  const [guide, pairing, installerActions] = await Promise.all([
    ...
    readFile(new URL("../src/components/installer-platform-actions.tsx", import.meta.url), "utf8"),
  ]);
  assert.match(guide, /InstallerPlatformActions/);
  assert.match(guide, /SitePairingCode/);
  assert.match(installerActions, /Baixar MonitorIA 24\/7/);
  assert.match(installerActions, /Abrir na Microsoft Store/);
  assert.match(installerActions, /Baixar Linux x64/);
...
test("download oficial usa release versionada atual", () => {
  ...
  assert.match(installerUrlFor("windows"), new RegExp(`/releases/download/agent-v${CURRENT_AGENT_VERSION}/MonitorIA-Setup\\.exe$`));
  assert.match(installerUrlFor("linux-x64"), new RegExp(`/releases/download/agent-v${CURRENT_AGENT_VERSION}/monitoria-agent-linux-x64\\.tar\\.gz$`));
  assert.match(installerUrlFor("linux-arm64"), new RegExp(`/releases/download/agent-v${CURRENT_AGENT_VERSION}/monitoria-agent-linux-arm64\\.tar\\.gz$`));
