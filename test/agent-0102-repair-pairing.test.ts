...
test("repareamento 1.0.2 preserva configuração local das câmeras", async () => {
  const source = await readFile(new URL("../agent/src/v102/runtime-scheduler.ts", import.meta.url), "utf8");
...
test("repareamento acorda fila e heartbeat atual imediatamente", async () => {
  const source = await readFile(new URL("../agent/src/v102/runtime-scheduler.ts", import.meta.url), "utf8");
  ...
  assert.match(source, /void tickScheduledHeartbeat\(this\)/);
  assert.match(source, /pairingRefreshesV102Immediately: true/);
  assert.match(source, /preservesLocalCameraStateOnRepair: true/);
});

test("autoteste 1.0.2 exige proteção de re-pareamento", async () => {
  const source = await readFile(new URL("../agent/src/index-v102.ts", import.meta.url), "utf8");
  assert.match(source, /scheduler\.preservesLocalCameraStateOnRepair !== true/);
  assert.match(source, /scheduler\.pairingRefreshesV102Immediately !== true/);
});
