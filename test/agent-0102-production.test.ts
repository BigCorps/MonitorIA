...
test("Agent e defaults de produção apontam para 1.0.1", async () => {
  const packageJson = JSON.parse(
    await readFile(new URL("../agent/package.json", import.meta.url), "utf8"),
  ) as { version: string };

  assert.equal(AGENT_VERSION, "1.0.1");
...
test("instalador Windows não exige terminal e inclui DPAPI nativo", async () => {
  ...
  assert.match(workflow, /AGENT_VERSION: "1\.0\.1"/);
  assert.match(linuxWorkflow, /AGENT_VERSION: "1\.0\.1"/);
});
