...
if (imagePaths.length < 1 || imagePaths.length > 4) {
  console.error("Uso: npm run analyze -- frame-inicial.jpg [frame-pico.jpg] [frame-final.jpg]");
  process.exit(1);
}
...
const labels: EventFrame["label"][] = ["start", "peak", "end", "extra"];
const now = new Date();
...
for (const [index, path] of imagePaths.entries()) {
  ...
  frames.push({
    ...
    capturedAt: new Date(now.getTime() + index * 10_000).toISOString(),
    imageUrl: `data:${mimeType(path)};base64,${bytes.toString("base64")}`,
  });
}
...
const result = await provider.analyzeEvent({
  ...
  profile: {
    cameraId,
    profileVersion: 1,
    environmentDescription: "Ambiente comercial visto por uma câmera estática.",
    monitoringGoals: [
      "identificar entrada e saída de pessoas e veículos",
      "descrever alterações relevantes no ambiente",
    ],
    ignoreInstructions: ["ignorar variações pequenas de iluminação"],
    ...
    zones: [
      {
        ...
        description: "Zona de teste cobrindo todo o quadro.",
      },
    ],
  },
  frames,
  localMetrics: {
    peakMotionPercent: 5,
    meanMotionPercent: 2,
    durationSeconds: Math.max(10, (frames.length - 1) * 10),
  },
});
