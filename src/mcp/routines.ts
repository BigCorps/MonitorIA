...
export async function getRoutineSummary(
  ...
  if (error) throw new Error("routine_summary_failed");
  ...
  return createEnvelope({
    ...
    limitations: [
      "O horário informado pelo usuário e o padrão aprendido são referências separadas.",
      "Uma diferença visual não prova causa, intenção ou falha operacional.",
      "Ausência de confirmação visual não prova que uma atividade não aconteceu.",
    ],
  });
}
