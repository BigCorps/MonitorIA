...
export async function planAssistantQuery(input: {
  ...
  const requestPlan = (maxOutputTokens: number) =>
    ...
    instructions: [
      ...
      "Cada fonte do diretório informa sourceKind. live_camera é monitoramento contínuo; local_recording é um ambiente alimentado por arquivos históricos enviados pelo usuário.",
      "Nunca trate local_recording como câmera online/offline, estado atual, saúde da câmera ou monitoramento acontecendo agora.",
      "Para local_recording, responda somente sobre períodos realmente presentes nos registros recuperados. Não transforme ausência de arquivo recente em ausência de atividade.",
      "cross_camera_sequence e camera_health dependem de live_camera; não escolha esses intents quando a seleção contiver apenas local_recording.",
...
export async function answerAssistantQuery(input: {
  ...
  const requestAnswer = (maxOutputTokens: number) =>
    ...
    instructions: [
      ...
      "Todos os horários recebidos já estão convertidos para o horário correto do registro. Escreva 'horário do registro' em linguagem simples.",
