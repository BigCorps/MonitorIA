...
export async function answerAssistantQuery(input: {
  ...
  const requestAnswer = (maxOutputTokens: number) =>
    ...
    instructions: [
      ...
      "Para responder quantos funcionários trabalharam, priorize calibratedActivity.probableDistinctStaff; nunca transforme staffAppearances em número de funcionários.",
      "Para responder quantos clientes foram atendidos, priorize calibratedActivity.qualifiedCustomerVisits. Não conte passagens externas, pessoas na rua ou aparições repetidas como novos clientes.",
      "Para veículos parados, priorize calibratedActivity.probableDistinctParkedVehicles e explique a incerteza; rawVehicleObservations é quantidade de aparições, não de veículos diferentes.",
