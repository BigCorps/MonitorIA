...
function detectDevice(): DeviceKind {
  ...
  if (/Android|iPhone|iPad|iPod|Mobile/i.test(ua) || touchMac) {
    return "mobile";
  }
  ...
  if (/Linux/i.test(ua) || /Linux/i.test(platform)) {
    if (/aarch64|arm64/i.test(`${ua} ${platform}`)) {
      return "linux-arm64";
    }
    return "linux-x64";
  }
...
function platformLabel(device: DeviceKind) {
  switch (device) {
    case "windows":
      return "Windows 64 bits detectado";
    case "linux-x64":
      return "Linux x86_64 detectado";
    case "linux-arm64":
      return "Linux ARM64 detectado";
    default:
      return "";
  }
}
...
function downloadHref(device: DeviceKind) {
  switch (device) {
    case "windows":
      return "/api/download-agent/windows";
    case "linux-x64":
      return "/api/download-agent/linux-x64";
    case "linux-arm64":
      return "/api/download-agent/linux-arm64";
    default:
      return null;
  }
}
...
function downloadLabel(device: DeviceKind) {
  switch (device) {
    case "windows":
      return "Baixar MonitorIA para Windows";
    case "linux-x64":
      return "Baixar MonitorIA para Linux x64";
    case "linux-arm64":
      return "Baixar MonitorIA para Linux ARM64";
    default:
...
export function InstallerPlatformActions({ compact = false }: Props) {
    ...
    return (
      <div className={styles.root}>
        <div className={styles.mobileCard}>
          ...
          <p>
            O Agent deve ser instalado em um computador que fique ligado e esteja
            na mesma rede local das câmeras, DVR ou NVR. Envie o link abaixo para
            esse computador.
          </p>
          <div className={styles.shareRow}>
            <button type="button" className={styles.primary} onClick={() => void shareInstallPage()}>
              Compartilhar link
            </button>
            <button type="button" className={styles.secondary} onClick={() => void copyInstallPage()}>
              {copied ? "Link copiado" : "Copiar link"}
            </button>
          </div>
        </div>
        {!compact ? (
          <a className={styles.otherPlatforms} href="/instalar">Ver plataformas disponíveis</a>
        ) : null}
      </div>
    );
  }
  ...
  return (
    <div className={styles.root}>
      ...
      <a className={styles.otherPlatforms} href="/instalar">
